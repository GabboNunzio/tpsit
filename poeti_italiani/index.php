...
.card-img{height:150px;background:var(--parchment);display:flex;align-items:center;position:relative;
...
function renderGrid(rows) {
  ...
  for(var i=0; i<rows.length; i++){
    ...
    var img = p.immagine_path
      ? '<img src="'+escH(p.immagine_path)+'" alt="'+escH(p.nome)+'">'
...
function openPoet(id) {
  ...
  apiFetch({action:'poeta', id:id}, function(err, p){
    ...
    if(p.immagine_path){
      portrait.innerHTML = '<img src="'+escH(p.immagine_path)+'" alt="">';
    } else {
